...
//     iosClientId:
//     "254552787517-g320ib7uilu8ssb14b0fl8o9r73ce6tv.apps.googleusercontent.com",
//   });
...
const SignupScreen = () => {
  const navigation = useNavigation();
  const [request, response, promptAsync] = Google.useAuthRequest({
    androidClientId: "254552787517-bvga6d8qlh4l56ehlhd69qgrl41kqsgo.apps.googleusercontent.com",
    iosClientId: "254552787517-g320ib7uilu8ssb14b0fl8o9r73ce6tv.apps.googleusercontent.com",
  });
  const { user, setUserInfo } = useOnBoardingContext();
  const { handleUserDetail, setUid } = userAuthUseContext();
  const [loading, setLoading] = useState(false);
  ...
  return (
    <View style={styles.container}>
      <SafeAreaView style={styles.innerContainer}>
        <TouchableOpacity style={[styles.button, { marginBottom: insets.bottom + 20 }]} onPress={promptAsync}>
          <View style={styles.iconContainer}>
            <GoogleG size={24} />
          </View>
          <Text style={styles.buttonText}>Sign in with Google</Text>
